...
namespace DCSBIOSDataBroker
{
        ...
        ISerialPortStatusListener, ISettingsDirtyListener, ISerialPortUserControlListener, IWindowsSerialPortEventListener, IDisposable
    {
        ...
        private const string WindowName = "DCS-BIOS Data Broker ";
        ...
        private async void CheckForNewRelease()
        {
            ...
            try
            {
                ...
                var client = new GitHubClient(new ProductHeaderValue("DCSBIOSDataBroker"));
                var timeSpan = DateTime.Now - dateTime;
                if (timeSpan.Days > 1)
                {
                    Settings.Default.LastGitHubCheck = DateTime.Now;
                    Settings.Default.Save();
                    var lastRelease = await client.Repository.Release.GetLatest("DCS-Skunkworks", "DCSBIOSDataBroker");
